#include <iostream>
#include "headers/ Linlist.h"
#include "headers/ Hash.h.h"
#include "headers/DinVec.h"
#include <cmath>
#include <string>
#include <cstdlib>
#include <sys/stat.h>
...
void createFilesInSubFolder(const json& table, const json& structure, const string& subName) {
    ...
    json tableArray = structure[table["name"]];
    int arrSize = tableArray.size();

    for (size_t i = 0; i < arrSize; i++) {
        columnNames.addtail(tableArray[i]);
    }
...
void createDataBase() {
    ...
    string DataBaseName = jsonData["name"];
    ...
    int tuplesLimit = jsonData["tuples_limit"];
